...
*)
  ...
  *)
  ...
  let use_constructor env ({ Location.loc; _ } as lid)
      (constr : Types.constructor_description) t =
    if record_usages then begin
      let t =
        ...
        | Longident.Ldot (lid, _) ->
          ...
          use_module env { Location.txt = lid; loc } path t
        | _ -> t
      in
  ...
  let use_label env ({ Location.loc; _ } as lid)
      (label : _ Types.gen_label_description) t =
    if record_usages then begin
      let t =
        (* When using a constructor, the modules appearing in its path should be
  ...
  let special_rule_for_aliases env { paths; substs } u_next path alias_lid =
    try
      ...
      if Hashtbl.mem already_used path' then ({ paths; substs }, u_next)
        ...
        (* TODO: check with Ulysse what rule this is *)
